    ...
    <div className="group rounded-2xl border border-border bg-[linear-gradient(180deg,rgba(255,255,255,0.05),rgba(255,255,255,0.02))] p-4 shadow-sm shadow-black/10 transition-all duration-200 hover:-translate-y-0.5 hover:border-white/15">
      <div className="flex items-start gap-3">
        <img
          ...
          loading="lazy"
          onError={(e) => {
            (e.currentTarget as HTMLImageElement).src = "/developers/default.png";
          }}
        />
      ...
      <div className="mt-3 flex flex-col gap-2 text-xs text-muted-foreground">
        ...
        <div className="flex items-center gap-2">
          ...
          <a
            href={`https://www.facebook.com/${member.facebook.split("/").filter(Boolean).pop()}`}
            target="_blank"
            rel="noreferrer"
      ...
      <div className="mt-3 flex items-center gap-2">
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="flex h-8 w-8 items-center justify-center rounded-full border border-border text-muted-foreground transition-colors hover:border-white/15 hover:text-foreground"
        >
          <Facebook size={14} />
        </a>
        <a
